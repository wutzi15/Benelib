...
template <class T> using add_const = const T;
...
template <class L, class V> using mp_fill = mp_transform<mp_constant<V>::template apply, L>;
...
template <class T, T N> using make_integer_sequence = typename make_int_seq_impl<T, 0, N>::type;
...
template< class T> void  print_type(std::string const & a, const std::string & b) 
{ 
